...
import { Card, CardHeader, CardBody, SectionLabel, Spinner } from '@/components/ui'
import { today } from '@/lib/utils'
import type { MealLog } from '@/types'

const VITAMINS = ['Vitamin D drops', 'Iron supplement', 'Multivitamin', 'Omega-3']
      ...
      <Card>
        ...
        <CardBody>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-1">
            {VITAMINS.map(v => {
              const done = !!(meals.vitamins ?? {})[v]
              return (
                <button key={v} onClick={() => toggleVitamin(v)}
                  className={`py-3 px-3 rounded-xl border-2 text-sm font-bold transition-all active:scale-95 text-center ${done ? 'bg-green-50 border-green-300 text-green-700' : 'bg-gray-50 border-gray-200 text-gray-400'}`}>
...
function MealTextarea({ value, onCommit, placeholder }: { value: string; onCommit: (v: string) => void; placeholder?: string }) {
