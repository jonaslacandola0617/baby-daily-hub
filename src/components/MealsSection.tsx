...
import {
  ...
  Star, Pencil, Clock, Tag, BookOpen, UtensilsCrossed,
} from 'lucide-react'
...
const emptyRecipeForm = {
  name: '', category: 'any' as RecipeCategory,
  description: '', prepTime: '', tags: '',
  ingredients: '', steps: '',
}
...
function MealLogTab({ date, qc }: { date: string; qc: ReturnType<typeof useQueryClient> }) {
    ...
    <>
      ...
      <Card>
        ...
        <CardBody>
          ...
          {vitaminList.length === 0 && <p className="text-sm text-gray-400 font-semibold py-2">No vitamins added — click Add above</p>}
...
function RecipeLibraryTab({ qc }: { qc: ReturnType<typeof useQueryClient> }) {
  ...
  const createMutation = useMutation({
    mutationFn: (data: typeof emptyRecipeForm) => axios.post('/api/recipes', data).then(r => r.data),
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<typeof emptyRecipeForm> }) =>
      axios.patch(`/api/recipes/${id}`, data).then(r => r.data),
    onSuccess: (updated: Recipe) => {
      qc.setQueryData(['recipes'], (old: Recipe[] = []) => old.map(r => r.id === updated.id ? updated : r))
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => axios.delete(`/api/recipes/${id}`).then(() => id),
    onSuccess: (id: string) => {
      qc.setQueryData(['recipes'], (old: Recipe[] = []) => old.filter(r => r.id !== id))
  ...
  function saveEdit(id: string) {
    if (!form.name.trim() || !form.ingredients.trim() || !form.steps.trim()) {
      toast.error('Name, ingredients, and steps are required'); return
    }
  ...
  const filtered = recipes.filter(r => {
    ...
    const matchSearch = !q || r.name.toLowerCase().includes(q) || (r.tags ?? '').toLowerCase().includes(q) || r.ingredients.toLowerCase().includes(q)
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-2">
        <div className="flex-1 relative">
          <BookOpen size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-300" />
          <input
        ...
        <Button onClick={() => { setAddingNew(true); setForm(emptyRecipeForm); setExpandedId(null) }} className="flex-shrink-0">
      ...
      {addingNew && (
        <RecipeForm
          ...
          setForm={setForm}
          onSave={submitNew}
          onCancel={() => { setAddingNew(false); setForm(emptyRecipeForm) }}
      ...
      {!addingNew && filtered.length === 0 && (
        <div className="py-12 text-center">
          ...
          <p className="text-sm font-bold text-gray-400">
            {recipes.length === 0 ? "No recipes yet — add Lia's favourites!" : 'No recipes match your search'}
          </p>
          {recipes.length === 0 && (
            <button onClick={() => { setAddingNew(true); setForm(emptyRecipeForm) }}
      ...
      {favourites.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-extrabold uppercase tracking-widest text-amber-500">⭐ Favourites</span>
          ...
          {favourites.map(recipe => (
            <RecipeAccordion key={recipe.id} recipe={recipe}
              ...
              onToggle={() => { setExpandedId(expandedId === recipe.id ? null : recipe.id); setEditingId(null) }}
              onEdit={() => startEdit(recipe)}
              onSaveEdit={() => saveEdit(recipe.id)}
              onCancelEdit={() => setEditingId(null)}
              onDelete={() => deleteMutation.mutate(recipe.id)}
              onToggleFav={() => toggleFavMutation.mutate({ id: recipe.id, isFavourite: !recipe.isFavourite })}
      ...
      {rest.length > 0 && (
        <div className="space-y-2">
          {favourites.length > 0 && (
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-gray-400">All recipes</span>
...
function RecipeAccordion({
  recipe, isExpanded, isEditing, form, setForm,
  onToggle, onEdit, onSaveEdit, onCancelEdit, onDelete, onToggleFav, isSaving,
}: AccordionProps) {
  const catInfo = CATEGORIES.find(c => c.value === recipe.category) ?? CATEGORIES[0]
  const tags = recipe.tags ? recipe.tags.split(',').map(t => t.trim()).filter(Boolean) : []
  const ingredients = recipe.ingredients.split('\n').filter(Boolean)
  const steps       = recipe.steps.split('\n').filter(Boolean)

  return (
    <div className={`bg-white rounded-2xl border transition-all ${isExpanded ? 'border-orange-200 shadow-sm' : 'border-orange-100'}`}>
      ...
      <div className="flex items-center gap-3 px-4 py-3.5">
        ...
        <button onClick={onToggle} className="flex-1 flex items-center gap-3 text-left min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-extrabold text-gray-800 truncate">{recipe.name}</p>
            <div className="flex items-center gap-2 mt-0.5 flex-wrap">
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${CAT_COLORS[recipe.category]}`}>
                {catInfo.label}
              </span>
              {recipe.prepTime && (
              ...
              {tags.slice(0, 2).map(tag => (
                <span key={tag} className="flex items-center gap-0.5 text-[10px] font-bold text-gray-400">
                  <Tag size={9} /> {tag}
                </span>
              ))}
            </div>
          ...
          {isEditing ? (
            <div className="px-4 py-4">
              <RecipeForm
                ...
                isSaving={isSaving} title="Edit recipe"
              />
            </div>
          ) : (
            <div className="px-4 py-4 space-y-4">
              {/* Description */}
              {recipe.description && (
                <p className="text-sm text-gray-500 font-semibold italic leading-relaxed">{recipe.description}</p>
              )}
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div>
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className="text-[10px] font-extrabold uppercase tracking-widest text-gray-400">Ingredients</span>
                  </div>
                  <ul className="space-y-1.5">
                    {ingredients.map((ing, i) => (
                      <li key={i} className="flex items-start gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-brand-300 flex-shrink-0 mt-1.5" />
                        <span className="text-sm font-semibold text-gray-700 leading-snug">{ing}</span>
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div>
                  <div className="flex items-center gap-1.5 mb-2">
                    <BookOpen size={12} className="text-ai" style={{ color: '#5B4FCF' }} />
                    <span className="text-[10px] font-extrabold uppercase tracking-widest text-gray-400">Steps</span>
                  </div>
                  <ol className="space-y-2">
                    {steps.map((step, i) => (
                      <li key={i} className="flex items-start gap-2.5">
                        <span className="w-5 h-5 rounded-full bg-orange-100 text-brand-500 text-[10px] font-extrabold flex items-center justify-center flex-shrink-0 mt-0.5">{i + 1}</span>
                        <span className="text-sm font-semibold text-gray-700 leading-snug">{step}</span>
                      </li>
                    ))}
                  </ol>
                </div>
              ...
              {tags.length > 0 && (
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {tags.map(tag => (
                    <span key={tag} className="text-[10px] font-bold bg-gray-100 text-gray-500 px-2.5 py-1 rounded-full">
                      #{tag}
                    </span>
                  ))}
                </div>
...
function RecipeForm({
  form, setForm, onSave, onCancel, isSaving, title,
}: {
  form:     typeof emptyRecipeForm
  setForm:  React.Dispatch<React.SetStateAction<typeof emptyRecipeForm>>
  onSave:   () => void
  ...
  return (
    <div className="bg-orange-50/60 rounded-2xl border border-orange-100 p-4 space-y-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        <Input placeholder="Recipe name *" value={form.name} onChange={f('name')} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        <div className="sm:col-span-2">
          <Input placeholder="Short description (optional)" value={form.description} onChange={f('description')} />
        </div>
        <Input placeholder="⏱ Prep time (e.g. 10 mins)" value={form.prepTime} onChange={f('prepTime')} />
      </div>
