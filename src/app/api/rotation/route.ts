...
export async function POST(req: NextRequest) {
  try {
    ...
    if (allToys.length === 0) {
      return NextResponse.json({ error: 'No toys found — add some toys first' }, { status: 400 })
    }
    if (allToys.length <= activeSetSize) {
      // If we have fewer toys than the active set size, just activate all
      await prisma.toy.updateMany({ data: { isActive: true, lastActiveAt: new Date() } })
    ...
    if (forcedActiveIds && forcedActiveIds.length > 0) {
      ...
      const allCategories = [...new Set(allToys.map(t => t.category))]

      newActiveIds = []

      // Step 1: add favourites first (up to activeSetSize)
      const favSlots = Math.min(favourites.length, Math.floor(activeSetSize / 2))
      ...
      const coveredCategories = new Set(
        newActiveIds.map(id => allToys.find(t => t.id === id)?.category).filter(Boolean)
      )
      ...
      while (newActiveIds.length < activeSetSize && remaining.length > 0) {
        ...
        const pool = remaining.length <= 2
          ? remaining
          : [...remaining.slice(0, mid), ...remaining.slice(0, mid), ...remaining.slice(mid)]
        const picked = pool[Math.floor(Math.random() * pool.length)]
        if (!newActiveIds.includes(picked.id)) {
          newActiveIds.push(picked.id)
          remaining.splice(remaining.findIndex(t => t.id === picked.id), 1)
        }
      }
    }
    ...
    await prisma.$transaction([
      ...
      // Activate new set
      ...newActiveIds.map(id =>
        prisma.toy.update({ where: { id }, data: { isActive: true, lastActiveAt: now } })
      ),
    ])
    ...
    // Return updated toys
    const updatedToys = await prisma.toy.findMany({ orderBy: [{ isActive: 'desc' }, { order: 'asc' }] })
    return NextResponse.json({ log, activeToys: updatedToys.filter(t => t.isActive), toys: updatedToys })
  } catch (error) {
    console.error('[ROTATION POST]', error)
    return NextResponse.json({ error: 'Failed to rotate toys' }, { status: 500 })
  }
}
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    settings = await prisma.rotationSettings.update({
      ...
      data: {
        ...(body.activeSetSize !== undefined && { activeSetSize: body.activeSetSize }),
        ...(body.scheduleDays  !== undefined && {
          scheduleDays:   body.scheduleDays,
          nextRotationAt: body.scheduleDays > 0 && settings.lastRotatedAt
...
async function updateNextRotation(settings: { id: string; scheduleDays: number } | null) {
  if (!settings) return
  const now = new Date()
  await prisma.rotationSettings.update({
    ...
    data: {
      lastRotatedAt:  now,
      nextRotationAt: settings.scheduleDays > 0 ? addDays(now, settings.scheduleDays) : null,
    },
  })
}
